  ...
  u1_session1_station1/
    frames/
      pov2/
        <ts>_pov2_rgb.png
        <ts>_pov2_depth.png
        <ts>_pov2.json        # sidecar pairing rgb + depth (like consumer)
    sensors/
      POVIMU/
...
UDP_IP = "127.0.0.1"
VIEW_RGB_PORT   = 9023
VIEW_DEPTH_PORT = 9024
RAW_IMU_PORT    = 9025  # reserved (not used here)
VIEW_IMU_PORT   = 9026  # reserved (not used here)
...
def ensure_session_layout(user_id: str, session_id: str, station_id: str) -> dict:
    ...
    pov_dir     = os.path.join(frames_dir, "pov2")
    ...
    if ENABLE_IMU:
        ...
        povimu_dir  = os.path.join(sensors_dir, "POVIMU2")
        ensure_dir(sensors_dir)
        ensure_dir(povimu_dir)
        imu_dirs = {"pov2": povimu_dir}
...
def imu_csv_paths(session: dict) -> dict:
    ...
    imu_dir = session["imu_dirs"].get("pov2")
    if not imu_dir:
        return {}
...
def start_pipeline_with_callback():
    """
    Start RealSense pipeline with a unified callback that receives:
      - framesets (COLOR + DEPTH)
      - IMU frames (gyro + accel)
    ...
    # If we get here, stop_evt was set
    return None, None
...
def restart_pipeline():
    ...
    with RESTART_LOCK:
        ...
        # Clear backlog so we don't process stale frames after reconnect
        try:
            q_frames.clear()
        except Exception:
            while q_frames:
        ...
        # Start a new pipeline (may wait while device is unplugged)
...
def control_server(stop_evt_local: Event):
    ...
    s.settimeout(0.2)
    print(f"🎛️  POV2 control listening on udp://{CONTROL_HOST}:{CONTROL_PORT}")
    ...
    try:
        while not stop_evt_local.is_set():
            ...
            if cmd.get("cmd") == "record_start":
                ...
                # Mark recording ON
                RECORDING = True
                print(f"⏺️  POV2 RECORDING START → {REC_META} | session_dir={CURRENT_SESSION['dir']}")
                ...
                print(
                    f"⏹️  POV2 RECORDING STOP (host_ts_end={HOST_TS_END}) "
    ...
    try:
        while not stop_evt.is_set():
            ...
            while q_frames:
                ...
                # 🔁 Rotar 180° porque la cámara está montada al revés
                ...
                if RECORDING and CURRENT_SESSION is not None:
                    ...
                    ts_clean = sanitize_ts_for_name(host_ts)
                    pov2_dir = session["frames"]["pov2"]
                    rgb_path   = os.path.join(pov2_dir, f"{ts_clean}_pov2_rgb.png")
                    depth_path = os.path.join(pov2_dir, f"{ts_clean}_pov2_depth.png")
            ...
            if now - t0 >= 1.0:
                print(
                    f"📤 POV2 stats: RGB={rgb_cnt}/s Depth={depth_cnt}/s "
